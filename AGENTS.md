...
| `packages/briar-node/src/services/xhsMediaService.ts` | 小红书自研解析（catsapi 兜底）：xhslink 短链跟 302 拿完整 URL（跳到裸首页=链接失效）→ GET 笔记页 HTML 抠 `window.__INITIAL_STATE__`（裸 undefined 字面量需替换再 JSON.parse）→ `note.noteDetailMap`：图文取 imageList[].urlDefault（无水印原图），livePhoto 标记的取 stream.h264 HD 档 masterUrl（实况视频），视频笔记取 video.media.stream.h264 HD 档。硬门槛 xsec_token（短链跳转自带，缺 token 被拦 300031 安全页）；媒体均 xhscdn http 链接，前端/代理升级 https |

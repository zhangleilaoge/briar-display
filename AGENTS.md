...
| `packages/briar-node/src/services/xhsMediaService.ts` | 小红书自研解析（catsapi 兜底）：xhslink 短链手动跟 302（老路径 discovery/item 会二次跳信息流丢笔记 ID，须改写为 explore；跳到裸首页=链接失效）→ GET 笔记页 HTML 抠 `window.__INITIAL_STATE__`（裸 undefined 字面量需替换再 JSON.parse）。**移动端 UA 优先**（noteData.data.noteData，能出网页端受限笔记；图片无 urlDefault 用 url 字段即原图），被按 IP 概率风控（/404/sec_ 页）隔 2s 换桌面端（note.noteDetailMap）兜底；图文 imageList[].urlDefault、实况 stream.h264 HD 档、视频 video.media.stream.h264 HD 档。硬门槛 xsec_token（短链跳转自带，缺 token 被拦 300031）；媒体均 xhscdn http 链接，前端/代理升级 https |

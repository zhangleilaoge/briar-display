...
export const certificateService = {
  ...
  async callDnsPodApi(
    ...
    if (!secretId || !secretKey) {
      console.warn(
        "⚠️  未配置 DNSPOD_SECRET_ID/DNSPOD_SECRET_KEY，跳过自动 DNS 更新",
      )
    ...
    try {
      ...
      if (statusCode !== 0) {
        ...
        throw new Error(
          `DNSPod API error (code ${statusCode}): ${message || "Unknown error"}`,
        )
      }
  ...
  async createDnsRecord(
    ...
    console.log(
      `   4. 添加新的 TXT 记录，名称为 "${subDomain}"，值为上面的内容`,
    )
    ...
    try {
      ...
      const challenges = await Promise.all(
        authorizations.map(async (authz) => {
          ...
          if (!challenge) {
            throw new Error(
              `未找到 dns-01 challenge for ${authz.identifier.value}`,
            )
          }
          ...
          const dnsRecord = Buffer.from(digest)
            ...
            .replace(/=/g, "")
          console.log(
            `DNS 记录值 (_acme-challenge.${authz.identifier.value}): ${dnsRecord}`,
          )
      ...
      console.log(
        "💡 如果 DNS 还未生效，验证可能会失败。可以 Ctrl+C 中断并重新运行",
      )
      ...
      console.log("正在验证挑战...")
      await Promise.all(
        challenges.map((c) => client.completeChallenge(c.challenge)),
      )
   ...
   * 上传证书到 CDN (COS)
   */
  async uploadCertificatesToCDN(
    domain: string,
  ): Promise<{ certUrl: string; keyUrl: string }> {
  ...
  async renewCertificate(domain: string): Promise<{
    ...
    try {
      ...
      // 3. 上传到 CDN
      const cdnUrls = await this.uploadCertificatesToCDN(
        domain.replace(/\*/g, "wildcard"),
      )

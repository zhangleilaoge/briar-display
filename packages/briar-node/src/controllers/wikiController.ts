import { type ApiResponse, HTTP_STATUS, type User } from '@briar/shared'
...
export const wikiController = {
	...
	async list(c: Context) {
		try {
			...
			const offset = Math.max(0, Number.isInteger(+offsetStr) ? Math.floor(+offsetStr) : 0)
	...
	async create(c: Context) {
		...
		try {
			...
			const message = error instanceof Error ? error.message : 'Failed to create wiki'
			const statusCode =
	...
	async delete(c: Context) {
		...
		try {
			...
			const message = error instanceof Error ? error.message : 'Failed to delete wiki'
			const statusCode =
				message === 'FORBIDDEN'
					? HTTP_STATUS.FORBIDDEN
					: message === 'NOT_FOUND'

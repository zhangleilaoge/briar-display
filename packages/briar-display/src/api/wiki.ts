...
export const wikiApi = {
	...
	async list(limit = 20, offset = 0) {
		try {
			const response = await apiClient.get<ApiResponse<WikiArticle[]>>('/wiki', {
				params: { limit, offset },
			})
	...
	async getMyWikis() {
		try {
			const response = await apiClient.get<ApiResponse<WikiArticle[]>>('/wiki/user/my')
			return response.data
		} catch (error) {
			return {
				success: false,
	...
	async getBySlug(slug: string) {
		try {
			const response = await apiClient.get<ApiResponse<WikiArticle>>(`/wiki/slug/${slug}`)
			return response.data
		} catch (error) {
			return {
				success: false,
	...
	async getById(id: string) {
		try {
			const response = await apiClient.get<ApiResponse<WikiArticle>>(`/wiki/${id}`)
			return response.data
		} catch (error) {
			return {
				success: false,
	...
	async create(payload: CreateWikiPayload) {
		try {
			const response = await apiClient.post<ApiResponse<WikiArticle>>('/wiki', payload)
			return response.data
		} catch (error) {
			return {
				success: false,
	...
	async update(id: string, payload: UpdateWikiPayload) {
		try {
			const response = await apiClient.put<ApiResponse<WikiArticle>>(`/wiki/${id}`, payload)
			return response.data
		} catch (error) {
			return {
				success: false,

...
> **AI 标识（强制）**：所有发表的评论必须带 AI 标识。通过 `briar-mr-review.sh` 的 `comment` / `reply` / `post-notes` 发表时，脚本会自动在评论**开头**加上 `> 🤖 *本评论由 AI 发布*`，body 中**不要**手动重复添加。若绕过原子脚本用临时 curl 脚本发表，必须手动在 body **开头**加上同样的标识。

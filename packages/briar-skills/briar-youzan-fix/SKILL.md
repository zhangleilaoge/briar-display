---
name: briar-youzan-fix
description: 用 Git worktree 隔离修复代码：按 MR 评论或 Pipeline 失败修复，验证后提交。通常由 briar-youzan-mr 触发。
---

# briar-youzan-fix: 代码修复工作流
...
| 来源 | 触发场景 |
|------|---------|
| briar-youzan-mr review | MR 中有实质性 comments 需要按评论修复代码 |
| briar-youzan-mr pipeline | Pipeline 中 lint/test/build 失败需要修代码 |
...
1. **本地已有目标仓库**（如果没有，先用 `briar-youzan-repo` 拉取）
...
2. **MR diff**（如果是 review 场景）：
   ...
   ```
3. **Comments 内容**：从 briar-youzan-mr 的 fetch 结果中读取
4. **Pipeline logs**（如果是 pipeline 场景）：从 briar-youzan-mr pipeline 的输出中读取失败原因；如果日志不足，使用 `zan-log-query` 或 `error-log-analyzer` 进一步查询天网日志。
...
```bash
./scripts/briar-youzan-fix.sh verify <worktree_path>
```
...
```bash
./scripts/briar-youzan-fix.sh diff <worktree_path>
```
...
```bash
# 1. 提交
./scripts/briar-youzan-fix.sh commit \
  <worktree_path> \
  "fix: 按 review 意见修复"

# 2. Push（如果用户要求）
./scripts/briar-youzan-fix.sh push <worktree_path>
```
...
## 与 briar-youzan-mr 的配合

### 在【修复评论】中使用

```
briar-youzan-mr fetch（获取 comments）
  ↓
逐条分析合理性
  ↓
需要修复的 → 调用 briar-youzan-fix
...
```
briar-youzan-mr pipeline（获取 pipeline jobs）
  ↓
...
| 验证修复 | `briar-youzan-fix` | `briar-youzan-fix.sh verify <worktree_path>` |
| 展示 diff | `briar-youzan-fix` | `briar-youzan-fix.sh diff <worktree_path>` |
| 提交 | `briar-youzan-fix` | `briar-youzan-fix.sh commit <worktree_path> "<msg>"` |
| push | `briar-youzan-fix` | `briar-youzan-fix.sh push <worktree_path>` |

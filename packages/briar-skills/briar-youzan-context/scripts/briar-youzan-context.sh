...
# briar-youzan-context.sh - 获取 URL 对应的页面上下文信息
# 用法: ./briar-youzan-context.sh <url>
...
if echo "$URL" | grep -qE 'jira\..*/browse/'; then
	...
	echo "[briar-youzan-context] Non-intranet URL, skipping."
	exit 0
fi

echo "[briar-youzan-context] Detected type: $TYPE"
echo "[briar-youzan-context] URL: $URL"
echo ""
...
if [ "$TYPE" = "fastbuild" ]; then
	...
	if [ -z "$TASK_ID" ]; then
		echo "[briar-youzan-context] 无法从 URL 中提取 fastbuild 任务 ID。"
		exit 1
	fi

	OPSCLI_OK=0
	if command -v opscli >/dev/null 2>&1; then
		echo "[briar-youzan-context] 使用 opscli 获取 fastbuild 任务 $TASK_ID..."
		STATUS_OUTPUT=$(opscli fastbuild status "$TASK_ID" 2>&1) && OPSCLI_OK=1
	fi

	if [ "$OPSCLI_OK" = "1" ]; then
		# 完整日志先落盘，避免大日志撑爆上下文
		LOG_FILE="${TMPDIR:-/tmp}/briar-youzan-context-fastbuild-${TASK_ID}.log"
	...
	echo "[briar-youzan-context] opscli 不可用或查询失败（可先执行 opscli login），降级为内网页面抓取..."
	TYPE="intranet"
fi
...
if [ "$TYPE" = "xiaolv-demand" ]; then
	...
	if [ -z "$DEMAND_ID" ]; then
		echo "[briar-youzan-context] 无法从 URL 中提取 Xiaolv 需求 ID。"
		exit 1
	fi

	echo "[briar-youzan-context] 使用 zan-skills/xiaolv-skill 查询需求详情..."
	...
	if ! echo "$RESPONSE" | jq -e '.code == 0' >/dev/null 2>&1; then
		...
		echo "[briar-youzan-context] Xiaolv API 查询失败: $MSG"
		exit 1
	fi
...
if [ "$TYPE" = "gitlab-mr" ]; then
	if [ -z "$GITLAB_TOKEN" ]; then
		echo "[briar-youzan-context] No GITLAB_TOKEN found."
	...
	echo "[briar-youzan-context] Fetching MR info via GitLab API..."
	...
	if [ -n "$JIRA_TICKET" ]; then
		echo ""
		echo "[briar-youzan-context] Found Jira ticket $JIRA_TICKET in MR description, fetching..."
...
if [ "$TYPE" = "jira" ]; then
	...
	if [ -z "$TICKET" ]; then
		echo "[briar-youzan-context] Could not extract ticket key from URL."
		exit 1
	fi
	...
	if [[ "$OSTYPE" == "linux-gnu"* ]]; then
		...
		if [ -n "$AUTH" ]; then
			echo "[briar-youzan-context] Fetching Jira via REST API (Linux)..."
			...
			if [ -n "$RESPONSE" ] && echo "$RESPONSE" | jq -e '.key' >/dev/null 2>&1; then
				...
				echo "[briar-youzan-context] Jira API failed, falling back..."
			fi
		else
			echo "[briar-youzan-context] No Jira credentials found."
	...
	if [[ "$OSTYPE" == "darwin"* ]]; then
		echo "[briar-youzan-context] Using Playwright + Chrome cookies (macOS)..."
		...
		echo "[briar-youzan-context] Playwright approach failed (exit $PY_EXIT), falling back to AppleScript..."
	...
	if [[ "$OSTYPE" == "darwin"* ]]; then
		echo "[briar-youzan-context] Using AppleScript + Chrome..."

		if [ ! -d "/Applications/Google Chrome.app" ]; then
			echo "[briar-youzan-context] ERROR: Google Chrome not found."
			exit 1
		fi
...
APPLEEOF
		...
		if [ $? -ne 0 ]; then
			echo "[briar-youzan-context] AppleScript failed: $RESULT"
			echo "[briar-youzan-context] Hint: Make sure Chrome → View → Developer → Allow JavaScript from Apple Events is enabled."
			exit 1
		fi
	...
	echo "[briar-youzan-context] Failed to fetch Jira page."
	echo "Linux options:"
...
if [ "$TYPE" = "intranet" ] || [ "$TYPE" = "gitlab-wiki" ]; then
	...
	if [ "$CURL_STATUS" = "200" ] && ! echo "$CURL_BODY" | grep -qi "登录\|login\|sign in"; then
		echo "[briar-youzan-context] Public page, fetched via curl."
		echo "---RAW_BODY---"
		cat "$TMP_BODY"
		rm -f "$TMP_BODY"
		exit 0
	...
	# curl 失败或遇到登录页
	echo "[briar-youzan-context] Page requires login."
...
APPLEEOF
	...
	# Linux: 无更多自动方案
	echo "[briar-youzan-context] Linux: page requires login and no credentials available."
	echo "  Options: copy-paste content manually, or provide cookies."
	exit 1
fi

echo "[briar-youzan-context] Unknown type or unsupported URL."
exit 1

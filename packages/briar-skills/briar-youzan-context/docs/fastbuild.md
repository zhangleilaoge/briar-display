...
`opscli` 未安装或查询失败时，降级为通用内网页面流程（macOS Playwright + Chrome cookie → AppleScript fallback），见 [generic.md](generic.md)。`briar-youzan-context.sh` 会自动处理该降级。

---

## 输出结构

`briar-youzan-context.sh` 会把**完整日志先落盘**到本地文件，避免大日志撑爆上下文：
...
--- 构建日志 ---
完整日志已保存: ${TMPDIR}/briar-youzan-context-fastbuild-<task_id>.log（共 N 行）
# N ≤ 300：内联输出全部日志
...
- **briar-youzan-fix**：根据「仓库 + 分支 + commit」定位代码，根据日志末尾的错误堆栈定位失败原因（typecheck、lint、构建 OOM 等）。

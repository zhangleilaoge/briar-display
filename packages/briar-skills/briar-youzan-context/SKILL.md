---
name: briar-youzan-context
description: 获取 Jira/GitLab/内网页面内容，为 briar-youzan-mr、briar-youzan-fix 提供上下文。MR 链接无明确意图时归 briar-youzan-mr 处理。
---

# briar-youzan-context: Agent 上下文获取

## 概述

本 skill 用于从各种链接中提取**结构化上下文信息**，供其他 skill 使用。

**与 briar-youzan-mr 的分工**：
- `briar-youzan-context`：获取**背景信息/页面内容**（Jira 需求、内网文档、MR 的关联信息）
- `briar-youzan-mr`：处理**MR 业务操作**（review、comment、reply、fix、pipeline）

下游 skill 可以通过本 skill 获取：
- **briar-youzan-fix**：修复代码前获取 Jira 需求描述、MR diff 背景
- **briar-youzan-mr**：Review MR 前获取关联的 Jira 需求背景（MR 本身的评论/review 由 briar-youzan-mr 处理）
- 其他需要页面内容的场景
...
if echo "$URL" | grep -qE 'jira\..*/browse/'; then
    ...
    echo "Non-intranet URL, skipping briar-youzan-context."
    exit 0
fi
```
...
```bash
./scripts/briar-youzan-context.sh <url>
# 或在 PATH 中直接使用
briar-youzan-context.sh <url>
```

...
> **AI 标识（强制）**：所有发表的评论必须带 AI 标识。通过 `briar-youzan-mr-review.sh` 的 `comment` / `reply` / `post-notes` 发表时，脚本会自动在评论**开头**加上 `> 🤖 *本评论由 AI 发布*`，body 中**不要**手动重复添加。若绕过原子脚本用临时 curl 脚本发表，必须手动在 body **开头**加上同样的标识。
...
```bash
../scripts/briar-youzan-mr-review.sh fetch <domain> <project_path> <mr_iid>
# 或
../scripts/briar-youzan-mr.sh fetch <domain> <project_path> <mr_iid>
```
...
```bash
../scripts/briar-youzan-mr-review.sh comment <domain> <project_path> <mr_iid> "评论内容"
# 或
../scripts/briar-youzan-mr.sh comment <domain> <project_path> <mr_iid> "评论内容"
```
...
```bash
../scripts/briar-youzan-mr-review.sh reply <domain> <project_path> <mr_iid> <discussion_id> "回复内容"
# 或
../scripts/briar-youzan-mr.sh reply <domain> <project_path> <mr_iid> <discussion_id> "回复内容"
```
...
1. **（如只有分支名）先反查 MR iid**
   ...
   ```bash
   ../scripts/briar-youzan-mr-review.sh find <domain> <project_path> <source_branch>
...
2. **获取 diff 并评估复杂度**
   ...
   ```bash
   ../scripts/briar-youzan-mr-review.sh diff <domain> <project_path> <mr_iid>
   # 或
   ../scripts/briar-youzan-mr.sh diff <domain> <project_path> <mr_iid>
   ```
...
2. **（如需要）创建 Review Worktree 查看完整上下文**
   ...
   当判断需要完整代码上下文时，用 `git worktree` 原生命令创建隔离的 review worktree。
   ...
   然后在本地仓库创建基于 `$SOURCE_BRANCH` 的 worktree（放主仓库同级目录）：
   ```bash
   cd <本地仓库路径>
   git fetch origin
   git worktree add ../<repo>-review-<mr_iid> "$SOURCE_BRANCH"
   ```
...
6. **（如切了 worktree）清理 Worktree**
   ...
   如果第 2 步创建了 worktree，review 完成后（无论是否发表了评论）**立即用 `git worktree remove <path>` 清理**。
...
```bash
../scripts/briar-youzan-mr-review.sh comment <domain> <project_path> <mr_iid> "[AI Review] app/foo.ts:42 ..."
```
...
../scripts/briar-youzan-mr-review.sh post-notes <domain> <project_path> <mr_iid> /tmp/comments.json
```
...
> **代码修复由 briar-youzan-fix 处理**。本章节只描述判断标准和决策逻辑，实际修复操作见 [briar-youzan-fix 文档](../../briar-youzan-fix/SKILL.md)。
...
- **代码修复阶段**调用 `briar-youzan-fix`：创建 worktree → 读取 comments + diff 上下文 → 修复 → 验证 → 用户确认 → 提交 → 清理 worktree。
...
```bash
# 创建 worktree：git worktree add <path> <source_branch>
# 清理 worktree：git worktree remove <path>

# 验证
../../briar-youzan-fix/scripts/briar-youzan-fix.sh verify <worktree_path>

# 展示 diff 等用户确认
../../briar-youzan-fix/scripts/briar-youzan-fix.sh diff <worktree_path>

# 用户确认后提交
../../briar-youzan-fix/scripts/briar-youzan-fix.sh commit <worktree_path> "fix: 按 review 修复"
../../briar-youzan-fix/scripts/briar-youzan-fix.sh push <worktree_path>
```
...
用户给出 N 个 MR 链接/分支
  ↓
反查每个 MR 的 iid（briar-youzan-mr-review.sh find）
  ↓
并行创建 worktree 并生成报告（子 Agent）
  ↓
主 Agent 汇总报告，过滤评论
  ↓
批量发布 DiffNote（briar-youzan-mr-review.sh post-notes）
  ↓
对定位失败的评论降级为顶层 Note
```

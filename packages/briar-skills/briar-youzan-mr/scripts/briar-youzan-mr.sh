#!/bin/bash
# briar-youzan-mr.sh - GitLab MR 全能工具（总入口）
# 根据 action 路由到对应的原子脚本。
#
# Usage:
#   ./briar-youzan-mr.sh create  <domain> <project_path> <source_branch> <target_branch> <title> [description]
#   ./briar-youzan-mr.sh fetch         <domain> <project_path> <mr_iid>
#   ./briar-youzan-mr.sh comment       <domain> <project_path> <mr_iid> <body>
#   ./briar-youzan-mr.sh reply         <domain> <project_path> <mr_iid> <discussion_id> <body>
#   ./briar-youzan-mr.sh diff          <domain> <project_path> <mr_iid>
#   ./briar-youzan-mr.sh find          <domain> <project_path> <source_branch>
#   ./briar-youzan-mr.sh post-notes    <domain> <project_path> <mr_iid> <comments.json>
#   ./briar-youzan-mr.sh pipeline      <domain> <project_path> <mr_iid>
#   ./briar-youzan-mr.sh pending       [domain] [days]
#
# 注意：review/pipeline 所需 worktree 用 git worktree 原生命令创建/清理。
#
# 也可以直接调用原子脚本：
#   ./briar-youzan-mr-create.sh   ...
#   ./briar-youzan-mr-review.sh   ...
#   ./briar-youzan-mr-pipeline.sh ...
#   ./briar-youzan-mr-pending.sh  ...
...
if [ "$ACTION" = "pending" ]; then
	shift
	exec "${SCRIPT_DIR}/briar-youzan-mr-pending.sh" "$@"
fi
...
case "$ACTION" in
	create)
		exec "${SCRIPT_DIR}/briar-youzan-mr-create.sh" "$DOMAIN" "$PROJECT_PATH" "$@"
		;;
	fetch|comment|reply|diff|find|post-notes)
		exec "${SCRIPT_DIR}/briar-youzan-mr-review.sh" "$ACTION" "$DOMAIN" "$PROJECT_PATH" "$@"
		;;
	pipeline)
		exec "${SCRIPT_DIR}/briar-youzan-mr-pipeline.sh" "$DOMAIN" "$PROJECT_PATH" "$@"
		;;
	*)

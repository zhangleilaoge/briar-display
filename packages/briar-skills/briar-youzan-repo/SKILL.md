---
name: briar-youzan-repo
description: 仓库管理：拉取、更新、清理、创建/删除 worktree。为 briar-youzan-fix 提供隔离修复环境。
---

# briar-youzan-repo: 仓库管理工具
...
**拉取仓库**：统一由 `zan-gitlab` skill 处理，它支持通过应用名、URL、Dubbo 服务、npm 包等业务线索定位仓库并同步到本地。`briar-youzan-repo` 不再自行实现 pull。

**Worktree 管理**：没有独立的 worktree skill，直接使用 `git worktree` 原生命令（见「三、Worktree 管理」）；`briar-youzan-fix` 创建/清理 worktree 时同样用原生命令。

**与 briar-youzan-fix 的关系**：
- worktree 创建/清理：直接用 `git worktree` 原生命令（见「三、Worktree 管理」）
- `briar-youzan-fix` 负责 worktree 内的**代码修复**（verify、diff、commit、push）
- `briar-youzan-repo` 负责仓库本地更新和清理

---

## Token 管理

与 `briar-youzan-mr` 共用同一个 `.env` 文件：
...
| 更新仓库 | "更新 xxx"、"pull 一下" | `briar-youzan-repo` | `briar-youzan-repo.sh update <repo> [base_dir]` |
| 清理工作区 | "保持干净"、"清理 xxx" | `briar-youzan-repo` | `briar-youzan-repo.sh clean <repo> [base_dir]` |
...
# 1. stash 当前改动
git stash push -m "briar-youzan-repo auto-stash $(date +%s)"
...
```bash
./scripts/briar-youzan-repo.sh update <repo-name> [base_dir]
# 或在 PATH 中直接使用
briar-youzan-repo.sh update <repo-name> [base_dir]
```
...
```bash
./scripts/briar-youzan-repo.sh clean <repo-name> [base_dir]
# 或在 PATH 中直接使用
briar-youzan-repo.sh clean <repo-name> [base_dir]
```
...
`pull` 移除后，`briar-youzan-repo` 只操作本地已有仓库。`update`/`clean` 默认在 `$HOME/projects/<repo>` 查找，若仓库在其他位置，请显式传入 `base_dir`：

```bash
./briar-youzan-repo.sh clean briar-display "$HOME/work"
```
...
## 与 briar-youzan-fix 的配合

`briar-youzan-fix` 的工作流：

1. **创建 worktree**：`git worktree add`（见「三、Worktree 管理」）
2. **修复代码**：在 worktree 内执行
3. **验证/展示 diff/提交/push**：调用 `briar-youzan-fix.sh`
4. **清理 worktree**：`git worktree remove <path>`

`briar-youzan-repo` 仅负责本地仓库的更新和清理，不直接参与 worktree 管理。

`briar-youzan-fix` 保留的能力：

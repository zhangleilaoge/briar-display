...
# 手动触发远程 Nginx 配置部署
# 用法：./packages/briar-scripts/scripts/trigger-deploy-nginx.sh [branch]
# 默认触发 master 分支
...
# 4. 触发 workflow
echo ""
echo "正在触发 workflow: deploy-nginx.yml ($BRANCH) ..."
gh workflow run deploy-nginx.yml --ref "$BRANCH"

# 5. 获取最新运行链接
sleep 2
RUN_URL=$(gh run list --workflow=deploy-nginx.yml --limit 1 --json url --jq '.[0].url' 2>/dev/null || true)
...
if [ -n "$RUN_URL" ]; then
	...
	echo "https://github.com/$(gh repo view --json nameWithOwner --jq '.nameWithOwner')/actions/workflows/deploy-nginx.yml"
fi

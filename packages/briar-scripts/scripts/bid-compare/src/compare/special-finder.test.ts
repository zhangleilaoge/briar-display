...
describe('findSpecialParagraphs', () => {
	it('通用模板内容的非标度低于包含稀有型号的内容', () => {
		...
		const chunks: TextChunk[] = [
			{ id: 'common', doc: 0, page: 1, text: commonText },
			{ id: 'special', doc: 1, page: 1, text: specialText },
		]
		const result = findSpecialParagraphs(chunks)
		const common = result.find((p) => p.chunkId === 'common')
		const special = result.find((p) => p.chunkId === 'special')
	...
	it('目录类文本会被过滤', () => {
		const chunks: TextChunk[] = [
			{ id: 'a', doc: 0, page: 1, text: '第一章 编制依据 ....................... 1' },
			{ id: 'b', doc: 0, page: 2, text: '第二章 工程概况 ....................... 3' },
		]
		const result = findSpecialParagraphs(chunks)
	...
	it('太短的文本会被过滤', () => {
		const chunks: TextChunk[] = [{ id: 'a', doc: 0, page: 1, text: '短文本 XYZ-123' }]
		const result = findSpecialParagraphs(chunks)
		expect(result.length).toBe(0)
	})
})
